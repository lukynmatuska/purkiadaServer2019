...
try:
    ...
    def startLoadTable():
        global tableFile 
        tableFile = loadTable.TableToTxt()
        while True:
            time.sleep(60)
            tableFile.reload()
            
            
    tableThread = threading.Thread(name="TableToTxt", target=startLoadTable)
...
while True:
    try:
        def startLoadTxT():
            ...
            del accsNew
            users_file.close()
            logging.info("Acounts from txt file: {}".format(accs))

            
        txtThread = threading.Thread(name="TxT accs", target=startLoadTxT)
        txtThread.setDaemon(True)
        txtThread.start()
        break
    except:
...
class User():
    def scoreLoad(self):
        logging.info("Loading score for user {}".format(self.name))
        try:
...
#tableFile.users.append("admin")
default_acess_list = ["user", "admin"]
# vytvářím složky a dávám je do sebe
users = Directory("users", default_acess_list)
data = Directory("data", default_acess_list)

desktop = Directory("desktop", default_acess_list)
desktop.add(users)
...
Read_Me = File("read_me.txt", message, default_acess_list)
data.add(Read_Me)
...
root.add(desktop)
# root.add(data)
bin = Directory("bin", default_acess_list)
bin.add(data)

home = Directory("home", default_acess_list)
home.add(bin)
home.add(root)
# home.add(logs)
# home.add(f2)
...
while True:
    ...
    cThread.daemon = True
    cThread.start()
    logging.info("New client from {}:{}".format(a[0], a[1]))
